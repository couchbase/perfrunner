...
class ControlPlaneManager:
    ...
    def save_project_id(self, create_project_if_none: bool = True):
        ...
        if self.project_id is None:
            # Nothing to save: no project exists yet, and ConfigParser rejects None.
            # Reached by the cleanup that runs when a deploy failed before creating one.
            logger.info("No project ID to save in cluster spec file.")
            return
    ...
    def _generate_api_key(self, org_id: str, name: str) -> tuple[str, str, str]:
        """Create a control plane API key (an access/secret pair) in the given organization.

        Tries the legacy `/tokens` endpoint first, falling back to the v2 control plane
        endpoint when it isn't served: newer control plane builds have dropped `/tokens`
        and answer it with a 404. Once `/tokens` is retired everywhere, the fallback can
        become the only path.

        Note this deliberately does not use the public v4 API keys endpoint. v4 issues
        bearer tokens, whereas every non-v4 Capella call perfrunner makes is HMAC-signed
        with an access/secret pair (see APIAuth), so an access/secret key is required.
        """
        ...
        if resp.status_code == 404:
            logger.info(
                "Legacy API key endpoint is not served. "
                "Falling back to the v2 control plane endpoint."
            )
            resp = self._create_control_plane_api_key(org_id, name)
        raise_for_status(resp)

        payload = resp.json()
        access_key, secret_key = payload.get("access"), payload.get("secret")
        if not (access_key and secret_key):
            # The v2 endpoint names them differently: the key ID doubles as the access
            # key, and the secret comes back as "secretKey". Its "token" field is just
            # base64("<id>:<secretKey>"), so there is nothing extra to read from it.
            access_key, secret_key = payload.get("id"), payload.get("secretKey")
        if not (access_key and secret_key):
            # Deliberately reports field names only: the payload holds a live secret.
            raise ValueError(
                f"No access/secret key pair in API key response (fields: {sorted(payload)})"
            )

        return payload["id"], access_key, secret_key

    def _create_control_plane_api_key(self, org_id: str, name: str) -> requests.Response:
        """Create an organization-scoped API key via the v2 control plane endpoint."""
        url = f"{self.api_client.internal_url}/v2/organizations/{org_id}/apikeys"
        body = {
            "name": name,
            "organizationRoles": ["organizationOwner"],
            "keyType": "machine",
            "description": "Generated by perfrunner",
        }
        return self.api_client.do_internal_request(url, method="POST", params=json.dumps(body))
    ...
    @staticmethod
    def save_model_api_key(model_api_key_id: str, model_api_key: str):
        """Save the models API key in the temporary credentials file."""
        existing_creds = {}
        try:
            with open(CAPELLA_CREDS_FILE, "r") as f:
        ...
        model_keys.update({"model_api_key_id": model_api_key_id, "model_api_key": model_api_key})
    ...
    def get_model_api_key() -> str:
        """Get the models API key from the temporary credentials file."""
        if not os.path.isfile(CAPELLA_CREDS_FILE):
            return ""

        with open(CAPELLA_CREDS_FILE, "r") as f:
            existing_creds = json.load(f)
        return existing_creds.get("model_api_keys", {}).get("model_api_key", "")

    @staticmethod
    def get_model_api_key_id() -> str:
        """Get the models API key ID from the temporary credentials file."""
        if not os.path.isfile(CAPELLA_CREDS_FILE):
            return ""

        with open(CAPELLA_CREDS_FILE, "r") as f:
            existing_creds = json.load(f)
        return existing_creds.get("model_api_keys", {}).get("model_api_key_id", "")
...
class CapellaProvisionedDeployer(CloudVMDeployer):
    def __init__(self, infra_spec: ClusterSpec, options: Namespace):
        ...
        # Absent when a deploy failed before creating them. Tolerated so that the cleanup
        # that runs after a failed deploy can no-op instead of raising a KeyError.
        self.tenant_id = self.infra_spec.controlplane_settings.get("org")
...
class CapellaModelServicesDeployer(CapellaProvisionedDeployer):
    ...
    def create_model_api_key(self):
        ...
        # The API key created will have access to all models in the specified region
        ...
        ControlPlaneManager.save_model_api_key(
            api_key_data.get("keyId", ""), api_key_data.get("apiKey", "")
        )
